    ...
    <div class="wh-wrapper">
        <div class="wh-container">
            <div class="wh-footer-btn">
                <a href="javascript:createMail();" class="fbtn-matter col-xs-12">写邮件</a>
            </div>
        </div>
    </div>
</footer>
...
<script type="text/javascript">
         ...
         $.ajax({
             ...
             success: function(data){
                 ...
                 for(var i = 0; i < jsonData.data0.length; i++){
                     ...
                     result +='<li onclick="openDetail('+mailId+','+mailUserId+',\''+detailType+'\');">'
                     +'<strong class="forum-avatar" valign="middle">'
                     +'<img src="'+empPhotoSrc+'"/>'
                    // +'<span>'+jsonData.data0[i].mailpostername+'</span>'
                     +'</strong>'
                     +'<p>'
                     +'<a>'+readFlag+jsonData.data0[i].mailpostername+'&nbsp;&nbsp;'+attachmentName+jsonData.data0[i].mailsubject+'</a>'
    ...
    //打开收信或发信详情页面
    function openDetail(mailId,mailUserId,detailType){
    	window.location = detailUrl+"?mailId="+mailId+"&mailuserId="+mailUserId+"&detailType="+detailType+"&homePage=1";
    }
	...
	//打开新建信息
	function createMail(){
		window.location = "/defaultroot/mail/new.controller?homePage=1";
	}
</script>
