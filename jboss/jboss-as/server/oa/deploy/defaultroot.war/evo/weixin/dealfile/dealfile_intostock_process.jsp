    		...
    		<article class="wh-edit wh-edit-document">
    			<div class="wh-container">
	            	...
	            	<c:set var="remark"><x:out select="$doc//fieldList/remark/text()"/></c:set>
						<tr>
							<th>
								<c:if test="${ptMode == '1'}">入库仓库</c:if>
								<c:if test="${ptMode == '4'}">退货仓库</c:if>
							</th>
							<td>${stockName }</td>
						</tr>
						<tr>
							<th>
								<c:if test="${ptMode == '1'}">入库部门</c:if>
								<c:if test="${ptMode == '4'}">退货部门</c:if>
							</th>
							<td>${orgName } </td>
						</tr>
						<tr>
							<th>
								<c:if test="${ptMode == '1'}">入库类别</c:if>
								<c:if test="${ptMode == '4'}">退货类别</c:if>
							</th>
							<td>${typeName }</td>
						</tr>
						<tr>
							<th>
								<c:if test="${ptMode == '1'}">供货单位</c:if>
								<c:if test="${ptMode == '4'}">退货单位</c:if>
							</th>
							<td>${suppName }</td>
						</tr>
						<tr>
							<th>
								<c:if test="${ptMode == '1'}">入库日期</c:if>
								<c:if test="${ptMode == '4'}">退货日期</c:if>
							</th>
							<td>${ptDate }</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								...
								<c:if test="${not empty subTable}">value="${subTable}条子表数据"</c:if>
								 readonly="readonly" onclick="addSubTable();"/>
							</td>
						</tr>
						<!--子表信息end-->
