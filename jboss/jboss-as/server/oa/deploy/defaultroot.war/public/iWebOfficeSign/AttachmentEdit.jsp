<%@ taglib uri="/WEB-INF/tag-lib/struts-tiles.tld" prefix="tiles" %>
<%@ taglib uri="/WEB-INF/tag-lib/struts-nested.tld" prefix="nested" %>
<%@ taglib uri="/WEB-INF/tag-lib/struts-logic.tld" prefix="logic" %>
<%@ taglib uri="/WEB-INF/tag-lib/struts-template.tld" prefix="template" %>
...
<%
 ...
 // String rootPath = "defaultroot";
  ...
  String mServerName="public/iWebOfficeSign/attachmentServer.jsp";
  ...
  String tempModuleType=request.getParameter("tempModuleType")==null?moduleType:request.getParameter("tempModuleType");//模板的类型
  ...
  String isEncrypt = request.getParameter("isEncrypt")==null?"0":request.getParameter("isEncrypt");
  ...
  if(request.getParameter("docName").toLowerCase().endsWith(".docx")){
        mFileType=".docx";	// 默认为.xls文档
  } else if( request.getParameter("docName").toLowerCase().endsWith(".xls")){
      mFileType=".xls";	// 默认为.xls文档
  } else if(request.getParameter("docName").toLowerCase().endsWith(".xlsx")){
	 mFileType=".xlsx";	// 默认为.xls文档
  } else if( request.getParameter("docName").toLowerCase().endsWith(".ppt")){
    mFileType=".ppt";	// 默认为.xls文档
  } else if(request.getParameter("docName").toLowerCase().endsWith(".pptx")){
	 mFileType=".pptx";	// 默认为.xls文档
  } else if( request.getParameter("docName").toLowerCase().endsWith(".wps")){
	 mFileType=".wps";	// 默认为.xls文档
  }
...
//System.out.println("======================"+mTemplate+"==========================="+nowTemplate);
	...
	/*if ( mEditType.compareTo("0,0")==0){
	 ...
	 mFileName = request.getParameter("docName");
	 moduleType  = request.getParameter("moduleType");
     if (mFileType.compareTo(".doc")==0 || mFileType.compareTo(".docx")==0){
		 mWord="";
		 mExcel="disabled";
	 } else{
		 mWord="disabled";
...
if("1".equals(showEditButton)){
   ...
   if(".doc".equals(mFileType)||".wps".equals(mFileType) ||".docx".equals(mFileType)){
...
//显示隐藏痕迹按钮
if("1".equals(showSignButton)&&(".doc".equals(mFileType)||".wps".equals(mFileType)||".docx".equals(mFileType))){
...
//显示隐藏印章
if("0".equals(showTempSign)&&(".doc".equals(mFileType)||".wps".equals(mFileType)||".docx".equals(mFileType))){
  b_fun_hide_sign=true; 
}else if("1".equals(showTempSign)&&(".doc".equals(mFileType)||".wps".equals(mFileType)||".docx".equals(mFileType))){
...
}else if("0".equals(showTempHead)&&(".doc".equals(mFileType)||".wps".equals(mFileType)||".docx".equals(mFileType))){
  b_fun_hide_head=true;
}else if(".doc".equals(mFileType)||".wps".equals(mFileType)||".docx".equals(mFileType) ){
...
%>
  ...
  webform.WebOffice.WebSetMsgByName("isEncrypt","<%=isEncrypt%>");
...
<%if(b_but_formalPrint&&!mFileType.equals(".xls")&&!mFileType.equals(".xlsx")){%>
...
function SaveDocument(moduleType,isSaveHtmlImage,isSaveDocFile,isSaveHtml){
	...
	if(".doc"==("<%=mFileType%>")||".wps"==("<%=mFileType%>") || ".docx"==("<%=mFileType%>")){
   //webform.WebOffice.WebSetMsgByName("MyDefine1","自定义变量值1");  //设置变量MyDefine1="自定义变量值1"，变量可以设置多个  在WebSave()时，一起提交到OfficeServer中
	...
	}else if(".xls"==("<%=mFileType%>")||".xlsx"==("<%=mFileType%>")){
	...
	}else if(".ppt"==("<%=mFileType%>")||".pptx"==("<%=mFileType%>")){
	       webform.WebOffice.WebSetMsgByName("isDoc","false");//是否保存为DOC文件
            if (!webform.WebOffice.WebSave()){    //交互OfficeServer的OPTION="SAVEFILE"
                StatusMsg(webform.WebOffice.Status);
                return false;
...
if (vIndex==101){insertDoc();}
if (vIndex==102){SaveDocument('<%=moduleType%>','<%=isSaveHtmlImage%>','<%=isSaveDocFile%>','<%=isSaveHtml%>');document.all.webform.submit(); alert('保存成功！');
 // webform.WebOffice.WebOpen(); 
}
if (vIndex==103){alterPicture('TempHead');}
if (vIndex==104){showTrack();}
...
function showSign(){
	...
	if(!ShowSign.equals("-1") && !mFileType.equals(".xls")&& !mFileType.equals(".xlsx")){//判断是不是在已批文件的锁定状态，是锁定则不控制痕迹
...
<body bgcolor="#ffffff" onLoad="Load();" onUnload="UnLoad()">  <!--引导和退出iWebOffice-->
  ...
  <form name="webform" id="webform" method="post" action="" onSubmit="return SaveDocument('<%=moduleType%>','<%=isSaveHtmlImage%>','<%=isSaveDocFile%>','<%=isSaveHtml%>');" target="targetFrame">  <!--保存iWebOffice后提交表单信息-->
             ...
             <tr>
                <td bgcolor=menu valign="top">
                 ...
                 调用iWebOffice，注意版本号，可用于升级-->
				  ...
				  <div  id="panel3"  name="panel3"  style="height:100%">
