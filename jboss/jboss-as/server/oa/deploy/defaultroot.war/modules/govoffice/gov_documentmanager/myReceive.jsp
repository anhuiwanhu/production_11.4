...
<body class="MainFrameBox" onload="init()">
	...
	<s:form name="queryForm" id="queryForm" action="GovRecvDocSet!myReceiveData.action" method="post" theme="simple">
	
	<!-- SEARCH PART START -->
	<%@ include file="/public/include/form_list.jsp"%>
	<input name="flag" type="hidden" value="notRead" class="btnButton4font"/>
    ...
    <%} else  if(!com.whir.common.util.CommonUtils.isForbiddenPad(request)){
	 ...
	 %>
		if(0==po.isReaded){
			return "<a href='javascript:void(0)' onclick='openWin({url:\"GovDocSendProcess!viewfile.action?sendFileUserId="+po.sendFileUserId+"&empId="+po.empId +"&p_wf_tableId="+po.tableId+"&p_wf_recordId="+po.id+"&canDownLoad="+po.canDownload+"\",width:620,height:290,isResizable: true,isFull: true,winName:\"shownoteread\"});' ><b><font color='red'>"+po.documentSendFileTitle+"</font></b></a>";
		}else{
			return "<a href='javascript:void(0)' onclick='openWin({url:\"GovDocSendProcess!viewfile.action?sendFileUserId="+po.sendFileUserId+"&empId="+po.empId +"&p_wf_tableId="+po.tableId+"&p_wf_recordId="+po.id+"&canDownLoad="+po.canDownload+"\",width:620,height:290,isResizable: true,isFull: true,winName:\"shownoteread\"});' >"+po.documentSendFileTitle+"</a>";
		}
	<%}%>
	}
