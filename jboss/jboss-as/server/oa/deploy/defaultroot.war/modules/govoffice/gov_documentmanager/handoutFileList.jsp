...
<body class="MainFrameBox" onload="init()">
	...
	<s:form name="queryForm" id="queryForm" action="GovDocSend!handoutFileListData.action" method="post" theme="simple">
	<input type=hidden name="numId" value="<s:property value="%{#request.parameters.numId}"/>"/>
	<input type=hidden name="toMe" value="<s:property value="%{#request.parameters.toMe}"/>"/>
	<input type=hidden name="numType" value="<s:property value="%{#request.parameters.numType}"/>"/>
	<!-- SEARCH PART START -->
    ...
    <!-- 操作按纽栏	-->
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="toolbarBottomLine" <%if("1".equals(request.getParameter("toMe"))){%>style="display:none"<%} %>>
	    <tr>
	        <td align="right">
	          <%if(!"1".equals(request.getParameter("toMe"))){%> <input name="" type="button" value="" class="btnButton4font" style="display:none"/>
			   <input name="getfile" type="button"  id="withdrawBtn" value="批量收藏" class="btnButton4font" onclick="collectionBatch()">
			   <input type="button" class="btnButton4font" onclick="ajaxBatchDelete('<%=rootPath%>/GovRecvDocSet!delMyReceive.action','id','id,sendFileUserId',this);" value="批量删除" />
			   <%}%>
	        </td>
	    </tr>
	</table>
    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="listTable">
		...
		<%if(!"1".equals(request.getParameter("toMe"))){%> 
			...
			<td whir-options="field:'writeOrg', width:'20%'">拟稿单位</td> 
			<td whir-options="field:'createdTime', width:'10%',renderer:common_DateFormat">发文日期</td> 
			<%if(!isCOSClient){%>
			<td whir-options="field:'transactStatus',width:'10%',renderer:downloadLink">下载正文</td> 
			<%}%>
	...
	function myOperate(po,i){
		...
		//var html =  '<img src="images/modi.gif" alt="修改" style="cursor:hand" onclick="modify(\''+po[0]+'\', \''+po[4]+'\', \''+po[5]+'\', \''+po[6]+'\', \''+po[7]+'\', \''+po[8]+'\')" ><img src="images/del.gif" alt="删除" style="cursor:hand" onclick="delSingle(\''+po[0]+'\')" >';
	...
	function show(po,i){
		...
		var html =  '<a href="javascript:void(0)" onclick="openWin({url:\'GovDocSendProcess!viewfile.action?toMe=<s:property value="%{#request.parameters.toMe}"/>&p_wf_recordId='+po.id+'&canDownLoad='+po.canDownload+'\',width:620,height:290,isResizable: true,isFull: true,winName:\'showUser\'});">'+po.title+'</a>';
		<%}else{%>
		var html =po.title;
		<%}%>
		return html;
...
%>
	function downloadLink(po,i){
		var url = '<%=smartInUse==1?"/defaultroot/public/download":fileServer%>/download.jsp?verifyCode='+po.verifyCode1+'&FileName='+po.gridId+po.wordType+'&name='+encodeURIComponent(po.title)+po.wordType+'&path=govdocumentmanager';
		if( po.canDownload == "0" ){
			return "";
		}else{
	...
	function myOperate(po,i){
		...
		var html =  '<a href="javascript:void(0)" onclick="ajaxDelete(\'GovRecvDocSet!delMyReceive.action?sendFileUserId='+po.userId+'&id='+po.id+'\',this);"><img border="0" src="<%=rootPath%>/images/del.gif" title="删除" ></a><a href="javascript:void(0)" onclick="back(\'sendFileUserId='+po.userId+'&empId='+po.empId +'&id='+po.id+'\');"><img border="0" src="<%=rootPath%>/images/undo.gif" title="退回" ></a>';
		<% if(mbd.hasRight(session.getAttribute("userId").toString(),"MYDOCUMENT2INFO*02*01")){%>
		if(po.isSyncToInfomation == "0"){
			html += '<a href="javascript:void(0)" onclick="InfoSynchronization(\''+po.id+'\',\''+po.byteNum+'\',\''+po.title+'\',\''+po.gridId+'\',\''+po.empId+'\',\''+po.accessoryName+'\',\''+po.accessorySaveName+'\');"><img border="0" src="<%=rootPath%>/images/changePwd.gif" title="同步到信息管理" ></a>';
		}
		<%}%>
	...
	function generateCHBX(po,i){
		...
		var html = 'sendFileUserId="'+ po.userId+ '" title="' + po.title + '"  url="' +  '/defaultroot/GovDocSendProcess!viewfile.action?viewonly=true&p_wf_tableId='+po.tableId+'&p_wf_recordId='+po.id+'"' ;
		//alert(html);
		return html;
	}
