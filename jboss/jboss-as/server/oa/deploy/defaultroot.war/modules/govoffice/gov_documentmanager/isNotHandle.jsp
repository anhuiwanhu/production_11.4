...
<body class="MainFrameBox" onload="init()">
	...
	<s:form name="queryForm" id="queryForm" action="GovRecvDocSet!isNotHandleData.action" method="post" theme="simple">
	
	<!-- SEARCH PART START -->
	<%@ include file="/public/include/form_list.jsp"%>
	<input name="tag" type="hidden" value="isNotHandle" class="btnButton4font"/>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="toolbarBottomLine">
	    <tr>
	        <td align="right">
				...
				<input type="button" class="btnButton4font" onclick="ajaxBatchOperate({url:'<%=rootPath%>/GovDocSet!notHandle.action',checkbox_name:'id',attr_name:'id,sendFileUserId',tip:'操作',isconfirm:true,formId:'queryForm',callbackfunction:null});" value="无需处理" />
			    <input type="button" class="btnButton4font" onclick="ajaxBatchOperate({url:'<%=rootPath%>/GovDocSet!delBatch.action',checkbox_name:'id',attr_name:'id,sendFileUserId',tip:'删除',isconfirm:true,formId:'queryForm',callbackfunction:null});" value="批量删除" /><!--ajaxBatchDelete('<%=rootPath%>/GovDocSet!delBatch.action','id','id,sendFileUserId',this);-->
	        </td>
	    </tr>
	</table>
	<%
    ...
    %>
        var url = '/defaultroot/public/download/download.jsp?verifyCode='+po.verifyCode1+'&FileName='+po.goldGridId+'.doc&name='+encodeURIComponent(po.documentSendFileTitle)+'.doc&path=govdocumentmanager';

            return "<a href='"+url+"' target='dwin'>"+po.documentSendFileTitle+"</a>";
