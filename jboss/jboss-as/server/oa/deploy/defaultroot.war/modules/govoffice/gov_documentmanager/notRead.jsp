...
<body class="MainFrameBox" onload="init()">
	...
	<s:form name="queryForm" id="queryForm" action="GovRecvDocSet!notReadData.action" method="post" theme="simple">
	
	<!-- SEARCH PART START -->
	<%@ include file="/public/include/form_list.jsp"%>
	<input name="tag" type="hidden" value="notRead" class="btnButton4font"/>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="SearchBar">  
        <tr>
            ...
            <td class="whir_td_searchinput">
				...
				<!-- <s:textfield id="queryOrg" name="queryOrg" value=""  size="16"  cssClass="inputText"  /> -->
            </td>
         ...
         <tr>
            ...
            <td colspan="4">
        		...
        		<!--<input type="checkbox" name="queryItem" id="goodDay0" value="0"/>-->
    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="listTable">
        ...
        <tr class="listTableHead">
			...
			<td whir-options="field:'documentSendFileTitle',width:'34%',renderer:getTitle">标题</td> 
			<td whir-options="field:'documentSendFileWriteOrg', width:'20%'">拟稿单位</td> 
			<td whir-options="field:'recivedDate', width:'8%',renderer:common_DateFormat">分发日期</td>
			<%if(!isCOSClient){%>
			<td whir-options="field:'goldGridId', width:'8%',renderer:downloadLink">下载正文</td>
			<%}%>
	...
	//自定义查看栏内容
	function show(po,i){
		var html =  '<a href="javascript:void(0)" onclick="openWin({url:\'viewUser.action?userId='+po.userId+'\',width:620,height:290,winName:\'showUser\'});">'+po.name+'</a>';
		return html;
	}
...
String.prototype.replaceAll = function(s1,s2){ return this.replace(new RegExp(s1,"gm"),s2); } 
	...
	function myOperate(po,i){
		...
		var html =  '<a href="javascript:void(0)" onclick="ajaxDelete(\'GovRecvDocSet!delMyReceive.action?sendFileUserId='+po.sendFileUserId+'&id='+po.id+'&canDownLoad='+ po.canDownload+'\',this);"><img border="0" src="<%=rootPath%>/images/del.gif" title="删除" ></a><a href="javascript:void(0)"  onclick="back(\'sendFileUserId='+po.sendFileUserId+'&empId='+po.empId +'&id='+po.id+'\');"><img border="0" src="<%=rootPath%>/images/undo.gif" title="退回" ></a>';
		<% if(mbd.hasRight(session.getAttribute("userId").toString(),"MYDOCUMENT2INFO*02*01")){%>
		if(po.isSyncToInfomation == "0"){
			html += '<a href="javascript:void(0)" onclick="InfoSynchronization(\''+po.id+'\',\''+po.documentSendFileByteNumber+'\',\''+po.documentSendFileTitle+'\',\''+po.goldGridId+'\',\''+po.empId+'\',\''+po.accessoryName+'\',\''+po.accessorySaveName+'\');"><img border="0" src="<%=rootPath%>/images/changePwd.gif" title="同步到信息管理" ></a>';
		}
		<%}%>
		return html;
	}
