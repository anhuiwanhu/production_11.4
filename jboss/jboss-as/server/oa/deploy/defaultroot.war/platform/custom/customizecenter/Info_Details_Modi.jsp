...
<body class="Pupwin" >  
	<div class="BodyMargin_10">    
		...
		<s:form name="dataForm" id="dataForm" action="/custinfo!updCustInfo.action" method="post" theme="simple" >  
           ...
           <%
                ...
                for(int j=0;j<_list.size();j++){
                   ...
                   %>
					 ...
					 <%
						  if((com.whir.component.util.Field.FIELD_AUTONUM+"").equals(fieldshow)){
					...
					%>
					  ...
					  <%}%>
					  <input type="hidden" id="old<%=fieldname%>" name="old<%=fieldname%>" value="<%=valueMap.get(fieldname)==null?"":valueMap.get(fieldname)+""%>">
	              </td>
                  <%}%>
...
//设置表单为异步提交  
initDataFormToAjax({"dataForm":'dataForm',"queryForm":'queryForm',"tip":'保存',"callbackfunction":returnSave});  
...
function returnSave(json){
	//window.opener.location.reload();
	 //window.opener.location.href="<%=rootPath%>/custinfo!seeDetailsCustInfo.action?InfoId=<%=InfoId%>&CENTERID=<%=CENTERID%>&TableId=<%=TableId%>&ModClassId=<%=ModClassId%>&rightscope=<%=rightscope%>&rightcodemanage=<%=rightcodemanage%>";
	 window.opener.reflush_DataForm();
}
